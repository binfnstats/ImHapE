...
OUTPUT_MODEL = 'OUTPUT_FOR_TRAINED_RNN_REGRESSION_MODEL/regression_' + str(REG) + '_' + FT + '_rnn.tf'
...
if FT in ['tajD_wuH', 'tajD_CNN', 'wuH_CNN', 'multi', 'CNN_all']:
    m = mod.trainLSTMreg(features = fts, fitness = fitness, multi = True, regularization = REG)
else:
    m = mod.trainLSTMreg(features = fts, fitness = fitness, regularization = REG)
